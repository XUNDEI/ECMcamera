...
def ping_ip(ip):
    try:
        output = subprocess.check_output(f'ping -n 4 -w 5000 {ip}', shell=True).decode('utf-8', errors='ignore')
        if "TTL=" in output:
            return True
...
def check_ip(ip):
    if ping_ip(ip):
        logging.info(f"{ip} 处于活动状态")
        mac_address = get_mac_from_arp(ip)
        if mac_address:
            logging.info(f"{ip} 的MAC地址: {mac_address}")
            return f"{ip} {mac_address}"
        else:
            logging.info(f"无法获取 {ip} 的MAC地址")
    else:
        logging.info(f"{ip} 没有响应ICMP请求")
    return None
